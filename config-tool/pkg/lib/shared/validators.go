...
var privateRanges = func() []*net.IPNet {
	...
	for i, b := range blocks {
		_, nets[i], _ = net.ParseCIDR(b)
	}
	return nets
}()

// isSafeHost resolves host and returns an error if any resolved address falls
// within a private or link-local range, preventing SSRF via config validators.
func isSafeHost(host string) error {
	...
	for _, addr := range addrs {
		...
		for _, cidr := range privateRanges {
			if cidr.Contains(ip) {
				return fmt.Errorf("host %q resolves to a private or reserved address (%s); connections to internal addresses are not permitted", h, addr)
			}
		}
	}
	return nil
...
func ValidateEmailServer(opts Options, mailServer string, mailPort int, useTLS bool, useAuth bool, username string, password string, fgName string) (bool, ValidationError) {
	...
	if err := isSafeHost(mailServer); err != nil {
		return false, ValidationError{
			Tags:       []string{"MAIL_SERVER"},
			FieldGroup: fgName,
			Message:    "MAIL_SERVER: " + err.Error(),
		}
	}
...
func ValidateGitLabOAuth(clientID, clientSecret, gitlabEndpoint string) bool {
	...
	// Credentials are sent in the POST body (not the URL) to avoid CWE-598
	// (secret exposure in server logs and browser history).
	body := url.Values{
		"client_id":     {clientID},
	...
	req, err := http.NewRequest("POST", gitlabEndpoint+"oauth/token", strings.NewReader(body.Encode()))
	if err != nil {
		return false
	}
	req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
...
func ValidateLDAPServer(opts Options, ldapUri, ldapAdminDn, ldapAdminPasswd, ldapUidAttr, ldapEmailAttr, ldapUserFilter string, ldapBaseDn []interface{}, fgName string) (bool, ValidationError) {
	...
	// Guard against SSRF: reject private/internal hosts before dialing.
	// Fail closed: any parse failure is treated as an unsafe URI.
	ldapParsed, parseErr := url.Parse(ldapUri)
	if parseErr != nil {
		return false, ValidationError{
			Tags:       []string{"LDAP_URI"},
			FieldGroup: fgName,
			Message:    "LDAP_URI: " + parseErr.Error(),
		}
	}
	if err := isSafeHost(ldapParsed.Host); err != nil {
		return false, ValidationError{
			Tags:       []string{"LDAP_URI"},
			FieldGroup: fgName,
			Message:    "LDAP_URI: " + err.Error(),
		}
	}
