...
class TestCrossLanguageThumbprint:
    ...
    def test_go_operator_thumbprint_matches_python(self):
        """
        Verify that Python's authlib produces the same RFC 7638 JWK thumbprint
        as the Go operator's jwkThumbprint() for a known RSA key.

        The PEM and expected kid below were generated by the Go code in
        controllers/quay/readonly.go (jwkThumbprint + bigIntBytes).  If this
        test fails, the operator-generated kid will not match what boot.py
        computes, and pods will crash during the read-only lifecycle.
        """
        go_generated_pem = b"""\
...
-----END RSA PRIVATE KEY-----"""
        ...
        go_computed_kid = "YlN2osAxoCv1QIkH2XvYpc9x2xXLZ4AHd6yOREinaME"

        python_kid = JsonWebKey.import_key(go_generated_pem).as_dict()["kid"]
        assert python_kid == go_computed_kid, (
            f"Python thumbprint {python_kid!r} != Go thumbprint {go_computed_kid!r}; "
            f"boot.py will reject operator-generated service keys"
        )
