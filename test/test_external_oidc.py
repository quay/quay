...
def test_sync_for_empty_oidc_groups(initialized_db):
    oidc_instance = fake_oidc()
    user_obj = model.user.get_user("devtable")

    test_org = model.organization.create_organization("testorg", "testorg@example.com", user_obj)
...
def test_sync_for_non_empty_oidc_groups(initialized_db):
    oidc_instance = fake_oidc()
    user_obj = model.user.get_user("devtable")
...
def test_resync_for_empty_quay_teams(initialized_db):
    oidc_instance = fake_oidc()
    user_obj = model.user.get_user("devtable")
...
def test_resync_for_non_empty_quay_teams(initialized_db):
    oidc_instance = fake_oidc()
    user_obj = model.user.get_user("devtable")
    fresh_user = model.user.get_user("freshuser")
    ...
    assert (
        TeamMember.select().where(TeamMember.user == user_obj, TeamMember.team == team_2).count()
        == 0
    )
