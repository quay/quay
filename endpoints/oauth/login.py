...
def _register_service(login_service):
    ...
    def attach_func():
        ...
        return redirect(url_for("web.user_view", _scheme=app.config["PREFERRED_URL_SCHEME"], _external=True, path=user_obj.username, tab="external"))
    ...
    def cli_token_func():
        ...
        user_obj = get_authenticated_user()
        return redirect(
            url_for("web.user_view", _scheme=app.config["PREFERRED_URL_SCHEME"], _external=True, path=user_obj.username, tab="settings", idtoken=idtoken)
        )
