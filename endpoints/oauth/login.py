...
def _perform_login(user_obj, service_name):
    ...
    if success:
        if model.user.has_user_prompts(user_obj):
            return redirect(url_for("web.updateuser", _scheme=app.config["PREFERRED_URL_SCHEME"]))
        else:
            return redirect(url_for("web.index", _scheme=app.config["PREFERRED_URL_SCHEME"]))
    else:
...
def _register_service(login_service):
    ...
    def attach_func():
        ...
        return redirect(url_for("web.user_view", _scheme=app.config["PREFERRED_URL_SCHEME"], path=user_obj.username, tab="external"))
    ...
    def cli_token_func():
        ...
        user_obj = get_authenticated_user()
        return redirect(
            url_for("web.user_view", _scheme=app.config["PREFERRED_URL_SCHEME"], path=user_obj.username, tab="settings", idtoken=idtoken)
        )
