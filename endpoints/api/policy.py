...
class UserAutoPrunePolicies(ApiResource):
    ...
    def get(self):
        ...
        policies = model.autoprune.get_namespace_autoprune_policies(user.username)
    ...
    def post(self):
        ...
        policy = model.autoprune.create_namespace_autoprune_policy(user.username, policy_config, create_task=True)
        return {"uuid": policy.uuid}, 201
...
class UserAutoPrunePolicy(ApiResource):
    ...
    def put(self, policy_uuid):
        ...
        policy = model.autoprune.update_namespace_autoprune_policy(user.username, policy_uuid, policy_config)
        if policy is None:
            raise NotFound()
        return policy_uuid, 204
