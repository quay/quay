...
func (inst *Installer) Install(ctx context.Context, cfg *Config) error {
	...
	if inst.HasInstallation() {
		return fmt.Errorf(
			"existing installation found at %s; run 'quay upgrade' to update it or 'quay uninstall' to remove it",
...
func (inst *Installer) Upgrade(ctx context.Context, cfg *Config) error {
	...
	if !inst.HasInstallation() {
		return fmt.Errorf("no existing installation found; run 'quay install' first")
	}
...
func (inst *Installer) resolveUpgradeDataDir(requestedDataDir string) (string, error) {
	...
	if filepath.Clean(requestedDataDir) != filepath.Clean(existing) {
		return "", fmt.Errorf(
