...
// repositoryGCQueuePattern matches the queue_name the API writes when marking
// a repository deleted: repositorygc/<namespace>/<repository id>/.
func repositoryGCQueuePattern(repoID int64) string {
	return "repositorygc/%/" + strconv.FormatInt(repoID, 10) + "/"
}
...
func (s *SQLiteStore) PurgeRepository(ctx context.Context, repo MarkedRepository) (RepositoryPurge, error) {
	...
	var queueID int64
	if repo.QueueID != "" {
		id, err := strconv.ParseInt(repo.QueueID, 10, 64)
		if err != nil {
			return purge, fmt.Errorf("malformed deletedrepository.queue_id %q: %w", repo.QueueID, err)
		}
		queueID = id
	}
	...
	steps := []struct {
		...
		{"stars", q.DeleteStarsByRepository},
	...
	if queueID != 0 {
		if err := q.DeleteQueueItem(ctx, daldb.DeleteQueueItemParams{
			ID:        queueID,
			QueueName: repositoryGCQueuePattern(repo.ID),
		}); err != nil {
			return purge, fmt.Errorf("delete queue item %d for repository %d: %w", queueID, repo.ID, err)
		}
	}
	deleted, err := q.DeleteRepository(ctx, repo.ID)
	if err != nil {
		return purge, fmt.Errorf("delete repository %d: %w", repo.ID, err)
	}
	if deleted == 0 {
		// FindMarkedRepositories ran outside this transaction. The repository
		// is gone (a concurrent cycle purged it) or is no longer in state 3.
		// Roll back so the dependent deletes above are not committed either.
		return purge, fmt.Errorf("repository %d: %w", repo.ID, ErrRepositoryNotMarked)
	}
