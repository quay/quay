...
func newUpgradeCmdWithDeps(upgrade func(context.Context, *installer.Config) int) *Command {
	...
	return &Command{
		...
		Description: "Updates the container image of an installed registry in place, preserving its data, hostname, and port. Fails if the registry is not installed; use 'quay install' first.",
		Flags:       fs,
