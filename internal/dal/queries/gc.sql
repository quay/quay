...
-- queue_id on the marker is free text and rowids are reused (no AUTOINCREMENT),
-- so the id alone could point at an unrelated item of another queue. Require the
-- repositorygc queue name for this repository as well.
DELETE FROM queueitem WHERE id = ? AND queue_name LIKE ?;
