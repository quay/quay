...
const getManifestByDigestWithMediaType = `-- name: GetManifestByDigestWithMediaType :one
...
type GetManifestByDigestWithMediaTypeParams struct {
	RepositoryID int64  `json:"repository_id"`
	Digest       string `json:"digest"`
}

type GetManifestByDigestWithMediaTypeRow struct {
	ManifestBytes string `json:"manifest_bytes"`
	MediaType     string `json:"media_type"`
}

func (q *Queries) GetManifestByDigestWithMediaType(ctx context.Context, arg GetManifestByDigestWithMediaTypeParams) (GetManifestByDigestWithMediaTypeRow, error) {
	row := q.db.QueryRowContext(ctx, getManifestByDigestWithMediaType, arg.RepositoryID, arg.Digest)
	var i GetManifestByDigestWithMediaTypeRow
	err := row.Scan(&i.ManifestBytes, &i.MediaType)
	return i, err
}
