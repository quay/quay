...
import pytest
from repomirror.skopeomirror import SkopeoMirror, create_authfile_content
...
def test_create_authfile_content():
    content = create_authfile_content(
        [
            ("quay.io", "user1:user1"),
            ("registry.redhat.io", "user2:user2"),
            ("docker.io", "anonymous"),
        ]
    )
    ...
    assert content.get("auths").get("docker.io").get("auth") == base64.b64encode(
        "anonymous".encode("utf8")
    ).decode("utf8")
...
def test_skopeo_command_tags():
    result = SkopeoMirror().tags(
        "docker://quay.io/quay/busybox", proxy={"http_proxy": "", "https_proxy": "", "no_proxy": ""}
    )
    assert result.success == True
