# -*- coding: utf-8 -*-
"""
URL validation utilities for user-supplied callback and external URLs.
"""
...
class SSRFBlockedError(ValueError):
    """Raised when a URL is rejected due to validation.
...
# Based on IANA Special-Purpose Address Registries.
BLOCKED_NETWORKS = [
...
def validate_external_registry_url(
    ...
    """
    Validate an external URL to prevent access to internal/private endpoints.
    ...
    Args:
        ...
        resolve_dns: Whether to resolve hostnames via DNS and check resulting IPs.
            ...
            be appropriate (e.g., data model layer). The API layer should always
    ...
    Raises:
        SSRFBlockedError: If the URL is blocked (blocked hostname, private/reserved
            IP, or DNS resolving to a blocked IP). Subclass of ValueError.
    ...
    if blocked_by_name and not name_allowlisted and not resolve_dns:
        logger.warning("Blocked: hostname '%s' is not allowed", hostname)
    ...
    try:
        ...
        if _is_ip_blocked(str(ip)):
            if not _is_allowed(hostname, str(ip), _allowed_hosts):
                logger.warning("Blocked: IP literal '%s' is in a blocked network", hostname)
    ...
    for ip_str in resolved_ips:
        if _is_ip_blocked(ip_str):
            if not _is_allowed(hostname, ip_str, _allowed_hosts):
                logger.warning(
                    "Blocked: hostname '%s' resolves to private/reserved IP '%s'",
                    hostname,
                    ip_str,
                )
