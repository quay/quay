...
during application startup, creating the system organization and robot
accounts before any authentication requests are processed.
"""
...
def initialize_kubernetes_sa_resources(config: dict) -> bool:
    ...
    create the system organization and robot accounts before any
    requests are processed.

    Creates:
    - System organization (e.g., 'quay-system') if it doesn't exist
    - Robot accounts for each ALLOWED_SUBJECTS entry
    ...
    try:
        ...
        # Create robot accounts for all configured allowed subjects
        allowed_subjects = service.allowed_subjects
        for allowed_subject in allowed_subjects:
            parsed = service.parse_sa_subject(allowed_subject)
            if not parsed:
                logger.warning(f"Invalid ALLOWED_SUBJECTS entry: {allowed_subject}")
                continue

            namespace, sa_name = parsed
            robot_shortname = service.generate_robot_shortname(namespace, sa_name)

            logger.info(f"Initializing robot for: {allowed_subject}")
