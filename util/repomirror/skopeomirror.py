...
def create_authfile_content(content: list = [namedtuple]) -> dict:
    return dict(
        auths=dict(
            map(
                lambda x: (
                    f"{x.location}",
                    {
                        "auth": f"{base64.b64encode(wrap_anonymous(x.username, x.password).encode('utf8')).decode('utf8')}"
                    },
                ),
                filter(lambda y: y.username != "", content),
            )
        )
    )
