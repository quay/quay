...
if [ "${CI:-false}" = "true" ] || [ "${PRE_COMMIT_STRICT:-false}" = "true" ]; then
  echo "::error::web/node_modules/.bin/eslint not found. Run 'cd web && pnpm install'." >&2
  exit 1
else
  echo "Warning: web/node_modules not found; skipping web ESLint. (Run 'cd web && pnpm install' to enable)" >&2
  exit 0
fi
