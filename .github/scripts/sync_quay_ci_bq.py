...
def setup_bigquery_tables(client: bigquery.Client):
    ...
    jobs_table.time_partitioning = bigquery.TimePartitioning(type_=bigquery.TimePartitioningType.DAY, field="started_at")
    ...
    tests_table.time_partitioning = bigquery.TimePartitioning(type_=bigquery.TimePartitioningType.DAY, field="execution_date")
...
def sync_ci_data(bq_client):
    ...
    while runs_url:
        ...
        for run in runs:
            ...
            if jobs_to_insert:
                errors = bq_client.insert_rows_json(f"{PROJECT_ID}.{DATASET_ID}.{JOBS_TABLE}", jobs_to_insert, row_ids=job_row_ids)
                if errors:
                    print(f"  -> Error inserting jobs for run {run_id}: {errors[:1]}")
                else:
                    print(f"  -> Inserted {len(jobs_to_insert)} jobs for run {run_id} (Branch: {branch_name}, PR: {pr_num})")
