...
class TestPopenExecutor:
    ...
    @patch("buildman.manager.executor.subprocess.Popen")
    @patch("buildman.manager.executor.LogPipe")
    def test_start_builder_tcp_docker_host(self, mock_logpipe_cls, mock_popen):
        mock_popen.return_value = MagicMock()
        executor = self._make_executor()

        with patch.dict("os.environ", {"DOCKER_HOST": "tcp://host.docker.internal:2375"}):
            executor.start_builder("token", "uuid")
    ...
    @patch("buildman.manager.executor.subprocess.Popen")
    @patch("buildman.manager.executor.LogPipe")
    def test_start_builder_warns_missing_unix_socket(self, mock_logpipe_cls, mock_popen):
        mock_popen.return_value = MagicMock()
        executor = self._make_executor()
