...
case "$ACTION" in
  ...
  check-version)
    echo "Checking fixVersion for ${ISSUE_KEY}..."
    if [ "$JIRA_CLI" = "acli" ]; then
      acli jira workitem view "$ISSUE_KEY" --fields fixVersions --json 2>/dev/null | \
        jq -r '.fields.fixVersions // [] | if length > 0 then "fixVersions: \([.[].name] | join(", "))\nBackporting REQUIRED after merge." else "No fixVersion set. Backporting not required." end' 2>/dev/null || {
        RESULT=$(jira_rest GET "issue/${ISSUE_KEY}?fields=fixVersions")
        echo "$RESULT" | jq -r '.fields.fixVersions // [] | if length > 0 then "fixVersions: \([.[].name] | join(", "))\nBackporting REQUIRED after merge." else "No fixVersion set. Backporting not required." end' 2>/dev/null
      }
    else
      RESULT=$(jira_rest GET "issue/${ISSUE_KEY}?fields=fixVersions")
      echo "$RESULT" | jq -r '.fields.fixVersions // [] | if length > 0 then "fixVersions: \([.[].name] | join(", "))\nBackporting REQUIRED after merge." else "No fixVersion set. Backporting not required." end' 2>/dev/null
  ...
  set-version)
    ...
    echo "Setting fixVersion on ${ISSUE_KEY} to '${VERSION}'..."
    jira_rest PUT "issue/${ISSUE_KEY}" "{\"fields\":{\"fixVersions\":[{\"name\":\"${VERSION}\"}]}}"
    echo "fixVersion set to '${VERSION}'."
    ;;
