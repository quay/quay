...
def fetch_tags_expiring_by_tag_count_policy(repo_id, policy_config, tag_page_limit):
    """
    ...
    while True:
        tags = oci.tag.fetch_paginated_autoprune_repo_tags_by_number(
            repo_id, int(policy_config["value"]), tag_page_limit
        )
        if len(tags) == 0:
            break
...
def fetch_tags_expiring_by_creation_date_policy(repo_id, policy_config, tag_page_limit):
    """
    ...
    while True:
        tags = oci.tag.fetch_paginated_autoprune_repo_tags_older_than_ms(
            repo_id, time_ms, tag_page_limit
        )
        if len(tags) == 0:
            break
        all_tags.extend(tags)
...
def fetch_tags_for_repo_policies(policies, repo_id):
    ...
    for policy in policies:
        if policy.method == AutoPruneMethod.NUMBER_OF_TAGS.value:
            tags = fetch_tags_expiring_by_tag_count_policy(repo_id, policy)
        elif policy.method == AutoPruneMethod.CREATION_DATE.value:
            tags = fetch_tags_expiring_by_creation_date_policy(repo_id, policy)
        if len(tags):
            all_tags.extend(tags)
    return all_tags
