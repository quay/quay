...
def _populate_blob(content, repository):
    content = Bytes.for_string_or_unicode(content).as_encoded_str()
    digest = str(sha256_digest(content))
    location = ImageStorageLocation.get(name="local_us")
    blob = store_blob_record_and_temp_link(
        repository.namespace_user.username, repository.name, digest, location, len(content), 120
    )
...
def _create_manifest_list(repository):
    ...
    _, config_digest = _populate_blob(layer_json, repository)

    # Add a blob of random data.
    random_data_1 = "foo"
    _, random_digest_1 = _populate_blob(random_data_1, repository)
    random_data_2 = "bar"
    _, random_digest_2 = _populate_blob(random_data_2, repository)
