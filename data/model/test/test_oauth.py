...
def test_robot_api_token_is_owned_by_robot_and_tracks_creator(initialized_db):
    ...
    token = api_token.create_token_under_limit(robot, creator, READ_REPO.scope, 3600, "CI token")

    assert token.subject_user == robot
    assert token.creator == creator
    assert [found.id for found in api_token.list_tokens(robot)] == [token.id]
    assert api_token.revoke_token(robot, token.uuid)
    assert api_token.list_tokens(robot) == []
