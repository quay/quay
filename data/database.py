...
def _db_from_url(
    ...
    if allow_pooling and os.getenv("DB_CONNECTION_POOLING", "false").lower() == "true" and parsed_url.drivername != "sqlite":
    ...
    if parsed_url.drivername == "sqlite":
        def _configure_sqlite_pragmas(db_instance):
            ...
            try:
                ...
                db_instance.execute_sql("PRAGMA journal_mode = WAL;")          # Most critical for concurrency
                db_instance.execute_sql("PRAGMA wal_autocheckpoint = 1000;")   # Manage WAL file size
                db_instance.execute_sql("PRAGMA synchronous = NORMAL;")        # Balance durability/performance
                ...
                busy_timeout_value = busy_timeout_result.fetchone()[0] if busy_timeout_result else "unknown"
                journal_mode_value = journal_mode_result.fetchone()[0] if journal_mode_result else "unknown"
                
                logger.info("Applied SQLite PRAGMA statements for database: %s - busy_timeout: %s, journal_mode: %s",
                        parsed_url.database, busy_timeout_value, journal_mode_value)
            except Exception as e:
                logger.warning("Failed to configure SQLite optimizations for %s: %s", 
                            parsed_url.database, e)
