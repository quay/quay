...
4. `templates/arches/{arch}.yaml`, then `templates/clouds/{cloud}-{arch}.yaml` -- only when `arch != amd64`; both are required and generation fails loudly if either is missing
5. `templates/tests/{test}.yaml`
6. `templates/presubmit/tests/{test}.yaml`, when present, merged on top of layer 5 (presubmit only); if it does not exist, presubmit uses layer 5 unchanged
...
`templates/arches/{arch}.yaml` adds the arch's OCP release (for example `releases.arm64-latest`) and the install override / node-arch env; `templates/clouds/{cloud}-{arch}.yaml` overlays the cloud's arch-specific machine type. `templates/base.yaml` and `templates/presubmit/base.yaml` keep `releases.latest.candidate.architecture` hard-coded to `amd64` -- that release is for CI tooling (build root, `cli` base image), which stays `amd64` regardless of the test cluster's arch. `templates/presubmit/base.yaml` also adds `capabilities: [arm64]` to the `quay-server` image item (not the Playwright runner) when `arch != amd64`, and only emits the `quay:latest` `promotion` block when `layout == base and arch == amd64` -- a non-`amd64` `layout: base` variant never promotes.
