...
def test_expand_matrix_cells() -> None:
    ...
    assert by_key[("3.18", "aws")].as_name == "aws-s3-3-18-nightly-4-22"
    assert by_key[("3.18", "gcp")].as_name == "gcp-gcs-3-18-nightly-4-22"
    assert by_key[("3.18", "azure")].as_name is None
...
def test_list_shows_phase0_row(capsys: object) -> None:
    ...
    assert "gcp-gcs-3-18-nightly-4-22" in out
    assert "gcp-gcs-3-17-daily-4-22" in out
    assert "azure-blob-3-16-daily-4-22" in out
