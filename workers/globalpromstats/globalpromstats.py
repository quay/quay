...
def populate_namespace_quota_stats():
    ...
    to prevent unbounded work, memory and metric cardinality on large registries.
    """
    ...
    namespace_quotas = {row["id"]: row for row in model.namespacequota.get_namespaces_with_quotas()}

    seen_namespace_ids = set()
    count = 0

    for namespace in model.quota.get_all_namespace_sizes():
        if count >= QUOTA_METRICS_NAMESPACES_LIMIT:
            logger.warning(
...
def populate_repo_quota_stats():
    ...
    At most QUOTA_METRICS_REPOS_LIMIT repositories are processed per cycle to
    prevent unbounded work, memory and metric cardinality on large registries.
    """
    repository_stats_used_bytes._metrics.clear()

    count = 0
    for repository in model.quota.get_all_repository_sizes():
        if count >= QUOTA_METRICS_REPOS_LIMIT:
            logger.warning(
