...
from workers.globalpromstats.globalpromstats import (
    GlobalPrometheusStatsWorker,
    _capped_quota_metrics_limit,
)

MockRegistrySize = namedtuple("MockRegistrySize", ["id", "queued", "running", "size_bytes"])


def test_capped_quota_metrics_limit():
    assert _capped_quota_metrics_limit(10000) == 10000
    assert _capped_quota_metrics_limit(1) == 1
    assert _capped_quota_metrics_limit(0) == 1
    assert _capped_quota_metrics_limit(-5) == 1
    assert _capped_quota_metrics_limit(999999) == 100000
    assert _capped_quota_metrics_limit("50") == 50
    assert _capped_quota_metrics_limit("bad", default=10000) == 10000
    assert _capped_quota_metrics_limit(None, default=10000) == 10000


@patch("workers.globalpromstats.globalpromstats.GlobalLock")
def test_try_report_stats_lock_not_acquired(mock_global_lock):
    from util.locking import LockNotAcquiredException

    mock_global_lock.return_value.__enter__.side_effect = LockNotAcquiredException()
    worker = GlobalPrometheusStatsWorker()
    worker._try_report_stats()  # should not raise


@patch("workers.globalpromstats.globalpromstats.GlobalLock")
def test_try_report_stats_acquires_lock(mock_global_lock):
    worker = GlobalPrometheusStatsWorker()
    with patch.object(worker, "_report_stats") as mock_report:
        worker._try_report_stats()
        mock_report.assert_called_once()
    mock_global_lock.assert_called_once_with("GLOBAL_PROM_STATS")


@patch("workers.globalpromstats.globalpromstats.GunicornWorker")
def test_create_gunicorn_worker(mock_gunicorn_worker):
    from workers.globalpromstats import globalpromstats as gps

    mock_gunicorn_worker.return_value = "gw"
    with patch.object(gps.app, "config", {"PROMETHEUS_PUSHGATEWAY_URL": "http://gw"}):
        assert gps.create_gunicorn_worker() == "gw"
...
class TestGlobalPrometheusStatsWorkerQuotaMetrics(unittest.TestCase):
    ...
    @patch(
        "workers.globalpromstats.globalpromstats.model.quota.get_all_namespace_quota_data",
        return_value=[
            {
                "id": 1,
                "username": "test_org1",
    ...
    def test_report_stats_with_quota_metrics(
        ...
        mock_get_all_namespace_quota_data,
    ...
    def test_namespace_metrics_limit(
        ...
        mock_get_all_namespace_quota_data.side_effect = lambda batch_size=500, max_rows=0: iter(
            all_ns[:max_rows] if max_rows > 0 else all_ns
        )
        ...
        with (
            ...
            # Verify max_rows was passed to the generator
            mock_get_all_namespace_quota_data.assert_called_with(max_rows=3)
    ...
    def test_disjoint_size_and_quota_namespaces(
        self,
        mock_get_all_repository_sizes,
        mock_get_all_namespace_quota_data,
        mock_get_registry_size,
        ...
        """All namespaces — size-only, quota-only, and both — must emit metrics."""
        with patch("workers.globalpromstats.globalpromstats.QUOTA_METRICS", True):
            worker = GlobalPrometheusStatsWorker()
            worker._report_stats()

            # size_only_org: has size, no quota
            self.assertEqual(
                REGISTRY.get_sample_value(
                    "quay_namespace_stats_used_bytes",
                    {"namespace": "size_only_org", "entity_type": "organization"},
                ),
                5000,
            )
            self.assertIsNone(
            ...
            # both_org: has both size and quota
            self.assertEqual(
                REGISTRY.get_sample_value(
                    "quay_namespace_stats_used_bytes",
                    {"namespace": "both_org", "entity_type": "organization"},
                ),
                8000,
            )
            self.assertEqual(
            ...
            # quota_only_org: has quota but no size row
            self.assertIsNone(
                REGISTRY.get_sample_value(
                    "quay_namespace_stats_used_bytes",
                    {"namespace": "quota_only_org", "entity_type": "user"},
                )
            )
            self.assertEqual(
                REGISTRY.get_sample_value(
    ...
    def test_partial_collection_preserves_gauges(
        self,
        mock_get_all_repository_sizes,
        mock_get_registry_size,
        mock_get_estimated_robot_count,
        ...
        """A generator that yields then raises must not clear existing gauges."""
        good_rows = [
            {
                "id": 1,
                "username": "stable_org",
        ...
        def fail_after_yield(batch_size=500, max_rows=0):
            yield {
                "id": 2,
                "username": "partial_org",
                "organization": True,
                "size_bytes": 999,
                "limit_bytes": 9999,
            }
            raise RuntimeError("DB failure after first row")

        with (
            patch("workers.globalpromstats.globalpromstats.QUOTA_METRICS", True),
            patch(
                "workers.globalpromstats.globalpromstats.model.quota.get_all_namespace_quota_data",
                return_value=good_rows,
            ),
        ):
            worker = GlobalPrometheusStatsWorker()
            worker._report_stats()
            self.assertEqual(
                REGISTRY.get_sample_value(
                    "quay_namespace_stats_used_bytes",
                    {"namespace": "stable_org", "entity_type": "organization"},
                ),
                5000,
            )
        ...
        with (
            ...
            # Previous successful cycle values must still be present
            self.assertEqual(
                REGISTRY.get_sample_value(
                    "quay_namespace_stats_used_bytes",
                    {"namespace": "stable_org", "entity_type": "organization"},
            ...
            # Partial row from the failed cycle must not have been published
            self.assertIsNone(
                REGISTRY.get_sample_value(
                    "quay_namespace_stats_used_bytes",
                    {"namespace": "partial_org", "entity_type": "organization"},
                )
            )
    ...
    def test_incomplete_backfill_omits_usage_metrics(
        self,
        mock_get_all_repository_sizes,
        mock_get_all_namespace_quota_data,
        mock_get_registry_size,
        ...
        """Namespaces with incomplete backfill (size_bytes=None) should emit
        quota capacity but not used_bytes or available_bytes."""
        with patch("workers.globalpromstats.globalpromstats.QUOTA_METRICS", True):
            worker = GlobalPrometheusStatsWorker()
            worker._report_stats()

            # Pending backfill: no used_bytes metric
            self.assertIsNone(
                REGISTRY.get_sample_value(
                    "quay_namespace_stats_used_bytes",
                    {"namespace": "backfill_pending_org", "entity_type": "organization"},
                )
            )
            # Pending backfill: capacity is emitted
            self.assertEqual(
                REGISTRY.get_sample_value(
                    "quay_namespace_quota_stats_capacity_bytes",
                    {"namespace": "backfill_pending_org", "entity_type": "organization"},
            ...
            # Pending backfill: available = capacity (used defaults to 0)
            self.assertEqual(
                REGISTRY.get_sample_value(
                    "quay_namespace_quota_stats_available_bytes",
                    {"namespace": "backfill_pending_org", "entity_type": "organization"},
            ...
            # Completed backfill: all metrics emitted normally
            self.assertEqual(
                REGISTRY.get_sample_value(
                    "quay_namespace_stats_used_bytes",
                    {"namespace": "backfill_done_org", "entity_type": "organization"},
                ),
                8000,
            )
            self.assertEqual(
