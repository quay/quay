...
| Backporting to `redhat-*` branches | `agent_docs/backports.md` |

## Universal Conventions

1. **Testing:** Every code change must include tests. The **Playwright suite** (`web/playwright/`) is Quay's end-to-end (e2e) blackbox test suite — it exercises the running system through its real UI and API surfaces, with no mocks or DB seeding (see `web/playwright/AGENTS.md`).

   **Frontend React changes: Playwright is required, not optional.** The `Web / e2e test coverage check` CI gate enforces this — it will fail on any React component change that ships without a corresponding Playwright spec. Rules:

   - **Required:** For any React frontend change that modifies user-visible component behavior (rendering, event handling, state, conditional UI, layout visible in the browser) — add or extend a Playwright spec in `web/playwright/e2e/`. This includes PatternFly prop renames, modal/button changes, form updates, and any other component-level work, even changes that appear narrow in scope.
   - **Vitest-only is acceptable** only for pure utility functions and data transformers that have no UI interaction at all (e.g., a helper that formats a string or transforms a data structure). It is **not** acceptable for component-level changes.
   - **If Playwright is genuinely infeasible** for a specific change (e.g., a code path that is entirely unreachable from the real UI), state the reason explicitly in the PR description. Do not silently omit Playwright and rely on the CI gate to catch the gap — explain the exception so reviewers can evaluate it.

   When fixing a bug, add or extend an e2e test that reproduces the original failure when feasible, so the regression stays caught. For backend: add pytest tests in the appropriate `test/` directory, and add or extend a Playwright `e2e/api/` test when the change affects externally observable API behavior. A backend-only change with no observable behavior change (internal refactor, logging, etc.) does not require Playwright. Always run relevant tests before committing.
