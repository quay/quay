...
import {isNullOrUndefined, humanizeTimeForExpiry, formatDate} from 'src/libs/utils';
...
export function useLogDescriptions() {
  ...
  // Helper to normalize timestamps that may be in milliseconds.
  ...
  const descriptions = {
    ...
    repo_mirror_config_changed: function (metadata: Metadata) {
      switch (metadata.changed) {
        ...
        case 'sync_start_date':
          ...
          return `Mirror ${metadata.changed} changed to ${formatDate(metadata.to)}`;
        case 'sync_interval':
    ...
    service_key_extend: function (metadata: Metadata) {
      const keyName = formatServiceKeyName(metadata);
      const oldDate = formatDate(metadata.old_expiration_date);
      const newDate = formatDate(metadata.expiration_date);
      return (
        <>
    ...
    change_tag_expiration: function (metadata: Metadata) {
      if (metadata.expiration_date && metadata.old_expiration_date) {
        ...
        const oldDate = formatDate(normalizeTimestamp(metadata.old_expiration_date));
