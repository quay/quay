...
export const securityDetailsCallStateSelector = selectorFamily<boolean, string>(
  {
    ...
    set:
      (digest: string) =>
      ({set, reset}, newSecurityDetailsCallState) => {
        if (newSecurityDetailsCallState instanceof DefaultValue) {
          reset(SecurityDetailsCallState(digest));
          set(SecurityDetailsCallStates, (prevValue: string[]) =>
            prevValue.filter((id: string) => id !== digest),
          );
        } else {
  ...
  get:
    ...
    ({get}) => {
      return digests.map((digest) =>
        get(securityDetailsCallStateSelector(digest)),
      );
    },
  set:
    (digests: string[]) =>
    ({get, set}, _) => {
      ...
      digests.forEach((digest) => {
        set(
          securityDetailsCallStateSelector(digest),
          !get(securityDetailsCallStateSelector(digest)),
        );
      });
    },
