...
export async function fetchNamespaceAutoPrunePolicies(namespace: string, signal: AbortSignal){
  const namespaceUrl = `/api/v1/organization/${namespace}/autoprunepolicy/`;
  // TODO: Add return type
...
export async function createNamespaceAutoPrunePolicy(namespace:string, policy: NamespaceAutoPrunePolicy) {
  const namespaceUrl = `/api/v1/organization/${namespace}/autoprunepolicy/`;
  const response = await axios.post(namespaceUrl, policy);
  assertHttpCode(response.status, 201);
}

export async function updateNamespaceAutoPrunePolicy(namespace:string, policy: NamespaceAutoPrunePolicy) {
  const namespaceUrl = `/api/v1/organization/${namespace}/autoprunepolicy/${policy.uuid}`;
  const response = await axios.put(namespaceUrl, policy);
  assertHttpCode(response.status, 204);
}

export async function deleteNamespaceAutoPrunePolicy(namespace:string, uuid: string) {
  const namespaceUrl = `/api/v1/organization/${namespace}/autoprunepolicy/${uuid}`;
