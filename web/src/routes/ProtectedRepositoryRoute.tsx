...
export default function ProtectedRepositoryRoute(): JSX.Element | null {
  ...
  // Anonymous users are rendered from StandaloneMain's early return (outside
  // the main <Page>), so they need their own Page wrapper with header/sidebar.
  // Authenticated users are already inside StandaloneMain's <Page>.
  if (user?.anonymous) {
    return (
      <Page
        masthead={<QuayHeader />}
