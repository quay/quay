...
export default function TagHistory(props: TagHistoryProps) {
    ...
    const {
        ...
        errorLoadingTags,
        lastUpdated,
    } = useAllTags(props.org, props.repo);
    const [page, setPage] = useState<number>(1);
    const [perPage, setPerPage] = useState<number>(10);
    
    // Memo these to prevent recalculating on every render
    const { tagList, tagEntries } = useMemo(() => processTags(tags, showFuture), [lastUpdated, showFuture]);
    const filteredTags = useMemo(() => {
        const fitleredByQuery: TagEntry[] = tagList.filter(tag => tag.tag.name.includes(query));
        const filteredByStartTime: TagEntry[] = !isNullOrUndefined(startTime) ? fitleredByQuery.filter(tag => tag.time >= startTime.getTime()) : fitleredByQuery;
        const filteredByEndTime: TagEntry[] = !isNullOrUndefined(endTime) ? filteredByStartTime.filter(tag => tag.time < endTime.getTime()) : filteredByStartTime;
        return filteredByEndTime;
    }, [lastUpdated, showFuture, query, startTime?.getTime(), endTime?.getTime()]);
    const isReadOnlyMode: boolean = quayConfig?.config?.REGISTRY_STATE === 'readonly';
            ...
            >
                <Thead>
                    <Tr>
                        ...
                        <Conditional if={!isReadOnlyMode && props.repoDetails?.can_write && quayConfig?.config?.PERMANENTLY_DELETE_TAGS}>
                            ...
                            <Conditional if={!isReadOnlyMode && props.repoDetails?.can_write && quayConfig?.config?.PERMANENTLY_DELETE_TAGS}>
                                <Td data-label="permanently-delete-tag">
                                    <Conditional if={
                                        ...
                                        [TagAction.Delete, TagAction.Revert].includes(tagEntry.action) &&
                                        props.repoDetails?.tag_expiration_s > 0 &&
                                        tagEntry.tag.end_ts > (new Date().getTime() / 1000) - props.repoDetails?.tag_expiration_s &&
...
function processTags(tags: Tag[], showFuture: boolean) {
    const MOVE_THRESHOLD: number = 2;
    const tagList: TagEntry[] = [];
    const tagEntries: Map<string, TagEntry[]> = new Map();

    const addEntry = (tag: Tag, time: number, action: TagAction, digest: string = null, oldDigest: string = null) => {
    ...
    const removeEntry = (tagEntry: TagEntry) => {
        ...
        if (tagEntries.has(tagEntry.tag.name)) {
            tagEntries.get(tagEntry.tag.name).splice(tagEntries.get(tagEntry.tag.name).indexOf(tagEntry), 1);
        }
    }
    ...
    for (const tag of tags) {
        ...
        if (tag.end_ts !== null && tag.end_ts !== undefined) {
            ...
            const futureEntry: TagEntry = currentEntries.length > 0 ? currentEntries[currentEntries.length - 1] : null;
            const futureTag: Tag = futureEntry !== null ? futureEntry.tag : null;
            if (futureEntry !== null && futureTag.start_ts - tag.end_ts <= MOVE_THRESHOLD) {
                removeEntry(futureEntry)
                addEntry(tag, tag.end_ts, futureTag.reversion ? TagAction.Revert : TagAction.Move, futureEntry.digest, tag.manifest_digest)
            } else {
        ...
        if (tag.start_ts !== null && tag.start_ts !== undefined) {
            addEntry(tag, tag.start_ts, tag.reversion ? TagAction.Recreate : TagAction.Create);
        }
    }
