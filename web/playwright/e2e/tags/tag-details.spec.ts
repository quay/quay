...
test.describe('Tag Details Page', {tag: ['@tags', '@container']}, () => {
  ...
  test('renders tag details with name, digest, size, and pull commands', async ({
    ...
    // Navigate to Manifest tab and verify content
    await authenticatedPage.getByRole('tab', {name: 'Manifest'}).click();
    await expect(authenticatedPage).toHaveURL(/tab=manifest/);
    await expect(authenticatedPage.locator('#manifest-content')).toContainText(
      'schemaVersion',
    );
    await expect(
      authenticatedPage.locator('#manifest-copy-button'),
    ).toBeVisible();
  });
...
test.describe(
  ...
  () => {
    ...
    test.beforeAll(async ({userContext, cachedContainerAvailable}) => {
      ...
      // Wait for Clair to scan the image (may 404 until indexed)
      const tags = await api.getTags(testRepo.namespace, testRepo.name);
      ...
      while (Date.now() < deadline) {
        try {
          ...
          // 404 until Clair indexes the manifest
        }
    ...
    test('vulnerability badge navigates to security report tab', async ({
      ...
      await expect(vulnBadge).toBeVisible({timeout: 10000});

      // Clair may report busybox as "Unsupported" — skip click test if so
      const vulnText = await vulnBadge.textContent();
      if (vulnText?.includes('Unsupported')) {
        return;
      }

      // Scan produced results — verify badge has severity and navigates
      await expect(vulnBadge).toHaveText(
