...
test.describe(
  ...
  () => {
    ...
    test.describe('Permissions', () => {
      test(
        'add write permission, update to admin, then remove',
        {tag: '@superuser'},
    ...
    test.describe('Collaborators', () => {
      test(
        'list collaborators under organization',
        {tag: '@superuser'},
        async ({superuserApi, adminClient}) => {
          const org = await superuserApi.organization();
          const repo = await superuserApi.repository(org.name);
    ...
    test.describe('Teams', () => {
      test(
        'create team, add repo permission, add member',
        {tag: '@superuser'},
        async ({superuserApi, adminClient}) => {
          const org = await superuserApi.organization();
          const repo = await superuserApi.repository(org.name);
    ...
    test.describe('Notifications', () => {
      test(
        'list notifications on empty repo returns 200',
        {tag: '@superuser'},
        async ({superuserApi, adminClient}) => {
          const org = await superuserApi.organization();
          const repo = await superuserApi.repository(org.name);
      ...
      test(
        'create, test, reset, get by uuid, and delete notification',
        {tag: '@superuser'},
        async ({superuserApi, adminClient}) => {
          const org = await superuserApi.organization();
          const repo = await superuserApi.repository(org.name);
    ...
    test.describe('Robot Accounts', () => {
      test(
        'create, get, and list robot accounts under org',
        {tag: '@superuser'},
        async ({superuserApi, adminClient}) => {
          const org = await superuserApi.organization();
      ...
      test(
        'admin can regenerate an org robot token',
        {tag: '@superuser'},
        async ({superuserApi, adminClient}) => {
    ...
    test.describe('Robot Federation', () => {
      test(
        'create with invalid issuer URL returns 400',
        {tag: '@superuser'},
        async ({superuserApi, adminClient}) => {
          const org = await superuserApi.organization();
          const robot = await superuserApi.robot(org.name);
      ...
      test(
        'create, get, and delete federation with valid issuer URL',
        {tag: '@superuser'},
        async ({superuserApi, adminClient}) => {
          const org = await superuserApi.organization();
          const robot = await superuserApi.robot(org.name);
    ...
    test.describe('Default Permissions (Prototypes)', () => {
      test(
        'create, list, update, and delete default permission for robot account',
        {tag: '@superuser'},
        async ({superuserApi, adminClient}) => {
          const org = await superuserApi.organization();
          const robot = await superuserApi.robot(org.name);
    ...
    test.describe(
      ...
      () => {
        test(
          'transition repo through MIRROR, READ_ONLY, and NORMAL states',
          {tag: '@superuser'},
          async ({superuserApi, adminClient}) => {
            const org = await superuserApi.organization();
            const repo = await superuserApi.repository(org.name);
...
test.describe(
  ...
  () => {
    test(
      'admin can invite user to team by email and revoke the invitation',
      {tag: '@superuser'},
      async ({superuserApi, adminClient}) => {
