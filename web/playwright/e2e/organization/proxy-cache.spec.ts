...
import {isContainerRuntimeAvailable, tryPushImage} from '../../utils/container';
...
test.describe(
  ...
  () => {
    ...
    test(
      ...
      async ({api}) => {
        // Skip if container runtime is not available
        const hasContainerRuntime = await isContainerRuntimeAvailable();
        test.skip(
          !hasContainerRuntime,
