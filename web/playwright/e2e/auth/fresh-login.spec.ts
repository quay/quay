...
test.describe(
  ...
  () => {
    test(
      'shows password modal (not redirect) on fresh_login_required',
      {tag: '@superuser'},
    ...
    test(
      'cancel dismisses the fresh-login modal',
      {tag: '@superuser'},
      async ({superuserPage: page}) => {
        await page.route('**/api/v1/superuser/logs*', (route) =>
          route.fulfill({
    ...
    test(
      'successful password verification dismisses modal and retries',
      {tag: '@superuser'},
      async ({superuserPage: page}) => {
        let callCount = 0;
