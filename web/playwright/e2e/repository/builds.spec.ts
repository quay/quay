...
test.describe(
  ...
  () => {
    ...
    test('build list sorts by date started', async ({
      ...
      // Trigger two builds with a delay to ensure different timestamps
      const build1 = await api.build(org.name, repo.name);
      await api.raw.waitForBuildPhase(org.name, repo.name, build1.buildId);

      const build2 = await api.build(org.name, repo.name);
