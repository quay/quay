...
test.describe(
  ...
  () => {
    ...
    test('superuser can create, view, and delete messages', async ({
      ...
      await expect(
        superuserPage.getByRole('heading', {name: 'Messages'}),
      ).toBeVisible();
      await expect(
      ...
      await expect(superuserPage.getByText(messageContent)).not.toBeVisible();
    });
    ...
    test('read-only superuser can access messages page', async ({
      ...
      await expect(
        readonlyPage.getByRole('heading', {name: 'Messages'}),
      ).toBeVisible();
